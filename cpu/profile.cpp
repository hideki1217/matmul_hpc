...
int main() {
  ...
  {
    const usize N = 256;
#define do_short(target) timeit(mmmul_profile<short>(N, N, N, target::mmmul<short>))
#define do_int(target) timeit(mmmul_profile<int>(N, N, N, target::mmmul<int>))
#define do_long(target) timeit(mmmul_profile<long>(N, N, N, target::mmmul<long>))
#define do_float(target) timeit(mmmul_profile<float>(N, N, N, target::mmmul<float>))
...
#undef do_float
  ...
  {
    const usize N = 1024;
#define do_short(target) timeit(mvmul_profile<short>(N, N, target::mvmul<short>))
...
#define do_float(target) timeit(mvmul_profile<float>(N, N, target::mvmul<float>))
