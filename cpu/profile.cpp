...
double _mmmul_profile(const usize M, const usize N, const usize K, F mmmul) {
  ...
  for (int i = 0; i < m; i++) mmmul(M, N, K, a.get(), b.get(), c.get());
  auto time_obj = std::chrono::system_clock::now() - start;
  time_ms_t time =
      std::chrono::duration_cast<std::chrono::milliseconds>(time_obj).count();

  const double op_Gn = M * N * K * m / 1e9;

  return op_Gn / (time / 1000.0);
}

template <typename T, typename F>
double _mvmul_profile(const usize M, const usize K, F mvmul) {
  auto a = std::make_unique<T[]>(M * K);
  ...
  auto start = std::chrono::system_clock::now();
  for (int i = 0; i < m; i++) mvmul(M, K, a.get(), b.get(), c.get());
  auto time_obj = std::chrono::system_clock::now() - start;
  time_ms_t time =
      std::chrono::duration_cast<std::chrono::milliseconds>(time_obj).count();
  const double op_Gn = M * K * m / 1e9;

  return op_Gn / (time / 1000.0);
}

template <typename T, typename F>
double mmmul_profile(const usize M, const usize N, const usize K, F f) {
  return _mmmul_profile<T>(M, N, K, std::function<mmmul_t<T>>(f));
}
template <typename T, typename F>
double mvmul_profile(const usize M, const usize K, F f) {
...
#define do_short(target) \
  gflopsit(mmmul_profile<short>(N, N, N, target::mmmul<short>))
#define do_int(target) gflopsit(mmmul_profile<int>(N, N, N, target::mmmul<int>))
#define do_float(target) \
  gflopsit(mmmul_profile<float>(N, N, N, target::mmmul<float>))
...
#undef do_float
  }
  {
    const usize N = 1024;
#define do_short(target) \
  gflopsit(mvmul_profile<short>(N, N, target::mvmul<short>))
#define do_int(target) gflopsit(mvmul_profile<int>(N, N, target::mvmul<int>))
#define do_float(target) \
  gflopsit(mvmul_profile<float>(N, N, target::mvmul<float>))
